...
void Scene::parse(std::string sceneDirectory, nlohmann::json sceneConfig)
{
  ...
  try
  {
    ...
    this->camera = Camera(
        Vector3f(cam["from"][0], cam["from"][1], cam["from"][2]), Vector3f(cam["to"][0], cam["to"][1], cam["to"][2]),
        Vector3f(cam["up"][0], cam["up"][1], cam["up"][2]), float(cam["fieldOfView"]), this->imageResolution);
  }
  catch (nlohmann::json::exception e)
  {
  ...
  try
  {
    ...
    for (std::string surfacePath : surfacePaths)
    {
      ...
      for (auto& s : surf)
      {
        this->bbox.min = Vector3f(std::min(this->bbox.min.x, s.bbox.min.x), std::min(this->bbox.min.y, s.bbox.min.y),
                                  std::min(this->bbox.min.z, s.bbox.min.z));

        this->bbox.max = Vector3f(std::max(this->bbox.max.x, s.bbox.max.x), std::max(this->bbox.max.y, s.bbox.max.y),
...
Interaction Scene::CamEmitterIntersect(Ray& ray)
{
  Interaction si;
  si.didIntersect = false;
  ...
  for (Light& light : this->lights)
  {
    Interaction siIntermediate = light.CamIntersect(&ray);
    if (siIntermediate.t <= ray.t && siIntermediate.didIntersect)
    {
      si = siIntermediate;
      ray.t = si.t;
...
Interaction Scene::dirnEmitterIntersect(Ray& ray)
{
  Interaction si;
  si.didIntersect = false;
  ...
  for (Light& light : this->lights)
  {
    Interaction siIntermediate = light.intersectDirnLight(&ray);
    if (siIntermediate.t <= ray.t && siIntermediate.didIntersect)
    {
      si = siIntermediate;
      ray.t = si.t;
