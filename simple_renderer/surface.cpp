  ...
  if (node.primCount != 0)
  {
    ...
    for (uint32_t i = 0; i < node.primCount; i++)
    {
      ...
      if (siIntermediate.t <= ray.t && siIntermediate.didIntersect)
      {
        ...
        si.a = si.n;
        int minIndex = (std::abs(si.n.x) < std::abs(si.n.y)) ? ((std::abs(si.n.x) < std::abs(si.n.z)) ? 0 : 2) :
                                                               ((std::abs(si.n.y) < std::abs(si.n.z)) ? 1 : 2);
        Vector3f perpendicularVector;
        perpendicularVector[minIndex] = 0.0;
        perpendicularVector[(minIndex + 1) % 3] = si.n[(minIndex + 2) % 3];
        perpendicularVector[(minIndex + 2) % 3] = si.n[(minIndex + 1) % 3];
        if (minIndex == 1)
          perpendicularVector[(minIndex + 1) % 3] *= -1;
        else
            perpendicularVector[(minIndex + 2) % 3] *= -1;
        si.b = perpendicularVector / perpendicularVector.Length();
        si.c = Cross(si.a, si.b);
